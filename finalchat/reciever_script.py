...
def recieving_file(c, file_name, key, iv):
    ...
    file_name = "reciever_recieved/"+file_name
    received_data = b''
...
def recieving_messages(c, key, iv):
    ...
    with open("reciever_private.pem", "rb") as f:
...
# Generating Reciever AES key
    Reciever_AES_key = get_random_bytes(16)
    Reciever_AES_iv = get_random_bytes(16)

    key_to_send = Reciever_AES_key+Reciever_AES_iv
    encrypted_message = rsa.encrypt(key_to_send, public_key)
    conn.sendall(encrypted_message)


# Recieving senders AES key
    encrypted_message = conn.recv(1024)
    decrypted_message = rsa.decrypt(encrypted_message, private_key)
    Sender_AES_key = decrypted_message[:16]
    Sender_AES_iv = decrypted_message[16:32]

    print("Successfully recieved and decrypted Sender AES key\n\n")

    print("Directly type the message to send the text.")
    print("To send data type File:path to file")
    print("all the recieved files are stored in reciever_recieved folder\n")

    threading.Thread(target=sending_messages, args=(
        conn, Reciever_AES_key, Reciever_AES_iv)).start()
    threading.Thread(target=recieving_messages, args=(
        conn, Sender_AES_key, Sender_AES_iv)).start()
